function [ir] = EPG_fitting_step1(x,inp,TEs,ret)
%%
...
% x     Input parameter matrix (T2, B1)
% inp   TSE data
...
alpha_in=x(2)*180;      %Input flip angle
T1_in=450;              %Set T1 to the approximate T1 over our post-mortem datasets - simulations reveal little sensitivity to this parameter in the T1 ranges found in our post-mortem analysis
...
if (refoc_mode == +1)                               % CPMG :
    ...
    %Omega_postRF(1,1) = sin(alpha_in/2);                          % magnetization resides on +x for fa=+90deg excitation
    %Omega_postRF(2,1) = sin(alpha_in/2);                          % remember: the Omega state matrix contains both F+(0) and F-(0)=(F+(0))*   
    %Omega_postRF(3,1) = cos(alpha_in/2);
...
%If ret !=1, return signal residual
if ret~=1
    ir=((ir-inp'));
end
