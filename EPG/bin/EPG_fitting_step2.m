function ir = EPG_fitting_step2(x,inp,B1,TEs,ret,scale,lambda)
%%
% This code estimates the T2 from the TSE signal via Extended
% Phase Graphs (EPG), with regularisation on T2

% Run from the EPG_wrapper_step2.m wrapper script
...
%%
%Output (ir) corresponds to the fitting residual or the simulated TSE signal

%Inputs are as follows:
% x         Input parameter matrix (T2)
% inp       TSE data
% TEs       Echo times (Echo spacing defined as on difference between first two echoes)
% ret       Function returns the summed difference between the TSE and fitted data (ret=0), or returns the simulated TSE signal (ret=1) - useful for evaluating quality of fit 
% scale     Normalised summed value of TSE data - used to ensure regularisation is correctly weighted relative to the signal amplitudes
% lambda    Regularisation factor for the median regularisation 

%%
%Define input parameters
T2_in=x(1);             %Input T2 estimate
alpha_in=B1*180;        %Define flip angle
T1_in=450;              %Set T1 to the approximate T1 over our post-mortem datasets - simulations reveal little sensitivity to this parameter in the T1 range found in our post-mortem analysis

%%
ESP_in=TEs(2)-TEs(1);   %Determine echo spacing
N_in=length(TEs);   %Define length of echo train
...
%%
%Normalise
ir=F0_vector_out./(sum(F0_vector_out(:).^2).^0.5);

%If ret !=1, return signal residual (scaled & with regularisation term, here returning the estimated T2 multiplied by lambda)
if ret~=1
    inp(1:end-1)=inp(1:end-1)*scale;
    ir=([scale*ir,T2_in*lambda]-inp');
end
